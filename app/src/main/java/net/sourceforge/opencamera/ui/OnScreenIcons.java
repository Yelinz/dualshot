...
public class OnScreenIcons {
    ...
    void addOnScreenIcons(List<View> buttons) {
        ...
        buttons.add(main_activity.findViewById(R.id.dual_camera_mode));
    }
    ...
    // DUALCAM-PIP begin
    /** Unlike the other update*Icon() methods, this reflects preference_dual_camera_pip (not
     *  ShowDualCameraModePreferenceKey, which only gates the icon's visibility - see
     *  showDualCameraModeIcon()) via alpha, matching how e.g. updateAutoLevelIcon() reflects
     *  its own pref, but using alpha since we only have a single ic_dual_camera drawable
     *  rather than separate "on"/"off" drawable assets. */
    private void updateDualCameraModeIcon() {
        ImageButton view = main_activity.findViewById(R.id.dual_camera_mode);
        boolean enabled = net.sourceforge.opencamera.dualcam.PipController.isPipEnabledPref(main_activity);
        view.setAlpha(enabled ? 1.0f : 0.5f);
        view.setContentDescription( main_activity.getResources().getString(enabled ? R.string.dualcam_pip_disable : R.string.dualcam_pip_enable) );
    }
    // DUALCAM-PIP end
    ...
    void setVisibility(int visibility, int visibility_video) {
        ...
        View dualCameraModeButton = main_activity.findViewById(R.id.dual_camera_mode);
        ...
        if( showDualCameraModeIcon() )
            dualCameraModeButton.setVisibility(visibility);
    }
    ...
    public boolean checkDisableGUIIcons() {
        ...
        if( !showDualCameraModeIcon() ) {
            View button = main_activity.findViewById(R.id.dual_camera_mode);
            changed = changed || (button.getVisibility() != View.GONE);
            button.setVisibility(View.GONE);
        }
    ...
    /** Whether to show the icon that toggles dual-camera PIP (a small front-camera window
     *  over the normal preview). Requires API 29+ (see net.sourceforge.opencamera.dualcam.PipController). */
    private boolean showDualCameraModeIcon() {
        if( Build.VERSION.SDK_INT < Build.VERSION_CODES.Q )
            return false;
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(main_activity);
        return sharedPreferences.getBoolean(PreferenceKeys.ShowDualCameraModePreferenceKey, true);
    }
    ...
    // DUALCAM-PIP begin
    /** Toggles dual-camera PIP (see net.sourceforge.opencamera.dualcam.PipController); called
     *  from MainActivity.clickedDualCameraMode(). Mirrors the clickedAutoLevel()-style pattern
     *  used by the other on-screen icons. */
    public void clickedDualCameraMode() {
        if( MyDebug.LOG )
            Log.d(TAG, "clickedDualCameraMode");
        if( Build.VERSION.SDK_INT < Build.VERSION_CODES.Q ) {
            // Defensive: the icon itself is only shown when supported (showDualCameraModeIcon()),
            // but this must never assume it wasn't invoked some other way.
            main_activity.getPreview().showToast(null, R.string.dual_camera_not_supported, true);
            return;
        }
        net.sourceforge.opencamera.dualcam.PipController.onToggled(main_activity);
        updateDualCameraModeIcon();
    }
    // DUALCAM-PIP end
