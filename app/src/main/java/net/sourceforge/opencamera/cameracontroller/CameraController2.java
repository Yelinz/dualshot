...
public class CameraController2 extends CameraController {
    ...
    final static int tonemap_log_max_curve_points_c = 64;
     ...
     */
    final static long max_preview_exposure_time_c = 1000000000L/5;
    ...
    private boolean hasCaptureSession() {
        if( camera == null ) // capture session shouldn't be started if camera not opened, but check just in case
            return false;
    ...
    /** Returns whether the current session type is for extension session.
     */
    boolean isExtensionSession() {
        return ( sessionType == SessionType.SESSIONTYPE_EXTENSION );
    }

    static RggbChannelVector convertTemperatureToRggbVector(int temperature_kelvin) {
    ...
    private final Camera2Settings camera_settings = new Camera2Settings(this);
    ...
    boolean supportsOpticalStabilization() {
    ...
    boolean supportsTonemapPresetCurve() {
        return this.supports_tonemap_preset_curve;
    }
...
//      Frame duration is in nanoseconds.  Using min to be safe.
        ...
        try {
            ...
            synchronized( background_camera_lock ) {
                if( video_recorder != null ) {
                    ...
                    // DUALCAM-PIP begin
                    // If PipController built a video-splice compositor for this recording (see
                    // the initVideoRecorderPostPrepare hook below), its camera-input surface
                    // stands in for MediaRecorder's own surface here: Camera2 streams the back
                    // camera into the compositor instead, which composites the front PIP on top
                    // and writes the final frames to MediaRecorder's surface itself
                    // (PIP_SPEC.md section E / PIP_RECON.md section 4).
                    Surface dualcam_pip_override = net.sourceforge.opencamera.dualcam.PipController.getVideoRecorderSurfaceOverride();
                    if( dualcam_pip_override != null ) {
                        video_recorder_surface = dualcam_pip_override;
                    }
                    // DUALCAM-PIP end
                }
                else
            ...
            try {
                if( use_fake_precapture_mode ) {
                    ...
                    if( want_flash ) {
                        ...
                        // or CONTROL_AE_MODE_ON_ALWAYS_FLASH even for fake flash (see note in Camera2Settings.setAEMode()) - and we
    ...
    public void initVideoRecorderPostPrepare(MediaRecorder video_recorder, boolean want_photo_video_recording) throws CameraControllerException {
        ...
        BLOCK_FOR_EXTENSIONS(); // not supported for extension sessions
        // DUALCAM-PIP begin
        // If PipController.prepareVideoSplice() armed a pending splice for this recording,
        // build the compositor now (MediaRecorder's surface is only valid post-prepare, i.e.
        // now) and publish its camera-input surface for the substitution above. No-op (and
        // essentially free) whenever no splice is armed - see PIP_RECON.md section 4.
        net.sourceforge.opencamera.dualcam.PipController.attachVideoCompositor(video_recorder.getSurface());
        // DUALCAM-PIP end
        try {
            if( MyDebug.LOG )
    ...
    boolean getPreviewIsVideoMode() {
        return this.previewIsVideoMode;
    }
    ...
    public void reconnect(boolean restart_preview) throws CameraControllerException {
        if( MyDebug.LOG )
            Log.d(TAG, "reconnect");
        // DUALCAM-PIP begin
        // Common teardown funnel for video recording stopping (both normal stop and error
        // paths reach here via Preview.videoRecordingStopped() -> reconnectCamera(), including
        // when stopped via onPause - see PIP_RECON.md sections 2 and 4): tear down any active
        // video-splice compositor and detach the front camera's compositor output. Idempotent
        // no-op when no splice was active. Must run before createCaptureSession() below
        // (whether or not restart_preview is true) so a stale override never leaks into a
        // later, unrelated capture session.
        net.sourceforge.opencamera.dualcam.PipController.onVideoRecordingStopped();
        // DUALCAM-PIP end
