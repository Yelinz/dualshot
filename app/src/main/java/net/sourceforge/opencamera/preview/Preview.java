...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    private void mySurfaceDestroyed() {
        ...
        // DUALCAM-PIP begin
        net.sourceforge.opencamera.dualcam.PipController.onMainCameraClosing();
        // DUALCAM-PIP end
    ...
    private void cameraOpened() {
        ...
        if( camera_controller != null ) {
            ...
            // DUALCAM-PIP begin
            // Re-evaluate whether the dual-camera PIP should be showing now that (a possibly
            // new) camera has finished opening - e.g. it should hide if we're now on OC's own
            // front camera, or (re)appear if we've switched back to the back camera. No-op if
            // the PIP pref is off, activity isn't a MainActivity, or this isn't an API29+/
            // Camera2 context - see PipController.onMainCameraOpened() (takes a plain Activity
            // so Preview.java doesn't need a MainActivity import/dependency here).
            net.sourceforge.opencamera.dualcam.PipController.onMainCameraOpened(activity);
            // DUALCAM-PIP end
        }
    ...
    private void startVideoRecording(final boolean max_filesize_restart) {
        ...
        // DUALCAM-PIP: the "PIP not recorded in this video" decision/toast now happens after
        // prepareVideoSplice() below, which also covers gates that only it can evaluate (e.g.
        // landscape-held recording).
        ...
        if( info == null ) {
            ...
            // DUALCAM-PIP begin
            // Phase P2 "upright buffers" orientation strategy (PIP_SPEC.md section E): if the
            // splice will be used, swap the profile's width/height (iff the device needs a
            // 90/270 rotation to display upright) so the encoder's buffer itself is
            // upright-shaped - the compositor then draws upright content straight into it (its
            // GL draws use the SurfaceTexture transform matrix only, which is already
            // device-validated to produce upright content - DUALCAM_SPEC.md) and no rotation
            // metadata is needed (setOrientationHint(0) below). Must run before
            // copyToMediaRecorder() below, which reads videoFrameWidth/videoFrameHeight.
            final boolean dualcam_pip_splice_active = net.sourceforge.opencamera.dualcam.PipController.prepareVideoSplice(
                    this, profile, getImageVideoRotation() % 180 == 90);
            if( net.sourceforge.opencamera.dualcam.PipController.isActive() && !dualcam_pip_splice_active ) {
                // Recording proceeds as stock OC video; tell the user once why the PIP they
                // see in the preview won't show up in this particular saved file.
                showToast(null, R.string.dualcam_pip_video_not_yet_supported, true);
            }
            // DUALCAM-PIP end
            ...
            try {
                ...
                // DUALCAM-PIP begin
                // See the "upright buffers" comment above copyToMediaRecorder(): when the
                // splice is active the encoder's buffer is already upright-shaped, so no
                // rotation metadata is needed (forcing a rotation on top would double-rotate).
                local_video_recorder.setOrientationHint(dualcam_pip_splice_active ? 0 : getImageVideoRotation());
                // DUALCAM-PIP end
                if( MyDebug.LOG )
